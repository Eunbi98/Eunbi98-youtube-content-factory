...
const MAX_CROSSFADE_SECONDS = 1.8;

const resolveTransitionSeconds = (
	timelineValue: number | undefined,
): number => {
	const requested = timelineValue ?? DEFAULT_CROSSFADE_SECONDS;
	return Math.min(
		MAX_CROSSFADE_SECONDS,
		Math.max(DEFAULT_CROSSFADE_SECONDS, requested),
	);
};
...
export const EpisodeComposition: React.FC<EpisodeCompositionProps> = ({
	...
	const captionColor =
		timeline.theme?.captionColor ?? ep005Theme.captionColor;
	...
	return (
		...
		>
			{timeline.scenes.map((scene, index) => {
				const previousScene =
					index > 0 ? timeline.scenes[index - 1] : undefined;
				const from = getSceneStartFrame(scene, fps);
				const durationInFrames = getSceneDurationFrames(scene, fps);
				const transitionSeconds = resolveTransitionSeconds(
					scene.transitionDuration,
				);
				const transitionDuration = secondsToFrames(
					transitionSeconds,
					fps,
				);
