...
const PreviousSceneLayer: React.FC<PreviousSceneLayerProps> = ({
	scene,
	...
	const safeTransitionDuration = Math.max(
		1,
		Math.floor(transitionDurationInFrames),
	);
	...
	return (
		<AbsoluteFill style={{opacity, zIndex: 0}}>
			<VisualLayer
				scene={scene}
				durationInFrames={safeTransitionDuration}
			/>
...
export const SceneRenderer: React.FC<SceneRendererProps> = ({
	...
	const safeDurationInFrames = Math.max(1, Math.floor(durationInFrames));
	const safeTransitionDuration = Math.max(
		1,
		Math.min(
			Math.floor(transitionDurationInFrames),
			Math.floor(safeDurationInFrames * 0.45),
		),
	);
	...
	const resolvedTransition: SceneTransitionConfig = {
		...transitionConfig,
		enableEnter: !isFirstScene && transitionConfig.enableEnter,
		enableExit: false,
	};
	...
	const shouldRenderPrevious =
		...
		resolvedTransition.preset !== 'cut';

	return (
		<AbsoluteFill>
			{shouldRenderPrevious && previousScene ? (
				<PreviousSceneLayer
					scene={previousScene}
					transitionDurationInFrames={safeTransitionDuration}
				/>
			) : null}
