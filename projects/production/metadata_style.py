...
METADATA_STYLE_INSTRUCTIONS = (
    ...
    "characters or fewer and append exactly ' #쇼츠 #shorts'. The description "
...
def validate_metadata_style(
    ...
    *,
    ...
    if not title.endswith(f" {TITLE_SUFFIX}"):
        raise MetadataStyleError(
            f"metadata.title은 ' {TITLE_SUFFIX}'로 끝나야 합니다."
        )

    title_hook = title[: -len(TITLE_SUFFIX)].strip()
    if not title_hook:
        raise MetadataStyleError("metadata.title의 호기심형 문장이 비어 있습니다.")
    if len(title_hook) > 42:
        raise MetadataStyleError(
            "metadata.title의 해시태그 앞 문장은 42자 이하여야 합니다."
        )
    if "\n" in title:
        raise MetadataStyleError("metadata.title에는 줄바꿈을 넣을 수 없습니다.")

    normalized_hook = _normalize_title(title_hook)
